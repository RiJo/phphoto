...
function phphoto_echo_admin_image($db, $image_id) {
    ...
    $sql = "SELECT id, width, height, type, filesize, filename, title, description FROM images WHERE id = $image_id";
    ...
    if (count($image_data) != 1) {
        echo "\n    <div class='error'>Invalid image id</div>";
        echo "\n</div>";
        return;
    }
